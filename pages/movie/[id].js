...
const DetailMovie = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${API_URL}movie/${router.query.id}?api_key=${API_KEY}&language=en-US&page=1`
      )
      ...
      {!isEmpty(detailMovie) ? (
        <>
          ...
          <Row className={styles.detailMovieWrap}>
            <Col md={12}>
              <h1 className={styles.title}>{detailMovie.title}</h1>
            ...
            <Col md={12}>
              ...
              <h1>Budget</h1>
              <p>{formatter.format(detailMovie.budget)}</p>
              <h1>Revenue</h1>
              <p>{formatter.format(detailMovie.revenue)}</p>
            </Col>
          </Row>
        </>
      ) : (
